...
export default function RolesTabOld() {
  const { 
    roles, 
    isLoading, 
    error, 
  ...
  if (error) {
    console.log('RolesTabOld - Error details:', error);
  ...
  return (
    <div className="flex flex-col h-full p-4 space-y-6">
      <div className="relative grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6" style={{ gridTemplateRows: 'repeat(auto-fill, minmax(200px, 1fr))', minHeight: '300px' }}>
        {/* Regular role cards */}
        {roles.map((role: Role) => (
          <RoleCard
            key={role.id}
            ...
            users={role.users || []}
            onViewPermissions={() => handleViewMore(role.id)}
          />
        ))}
